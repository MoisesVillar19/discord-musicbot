...
def now_playing_embed(track: dict) -> discord.Embed:
    ...
    if by:
        desc += f"\n🙋 Pedida por {by}"
    embed = discord.Embed(title="🎧 Now Playing", description=desc,
...
def queue_page_embed(page_tracks: list, page: int, total_pages: int,
                     total: int, offset: int) -> discord.Embed:
    lines = [f"{offset + i}. {track_line(t)}" for i, t in enumerate(page_tracks, start=1)]
    desc = "\n".join(lines) if lines else "🎵 Cola vacía."
    embed = discord.Embed(title="🎶 Cola actual", description=desc,
